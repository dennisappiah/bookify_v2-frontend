  ...
  if (searchQuery) {
    ...
    filteredBooks =
      currentFilter && currentFilter._id
        ? books.filter((book) => book.category._id === currentFilter._id)
        : books;
  }
          ...
          <table className="table">
            <thead>
              <tr>
                ...
                <th
                  scope="col"
                  onClick={() => handleSortByColumn("category.name")}
                >
                  Category {renderSortIcon("category.name")}
                </th>
                <th
                  scope="col"
                  onClick={() => handleSortByColumn("numberInStock")}
                >
                  Stock {renderSortIcon("numberInStock")}
                </th>
                <th
                  scope="col"
                  onClick={() => handleSortByColumn("dailyRentalRate")}
                >
              ...
              {paginatedBooks.map((book) => {
                return (
                  <Book
                    ...
                    key={(book as BookModel)._id}
                    onLike={likeBook}
                    onRemove={handleBookDelete}
                  />
                );
              })}
            </tbody>
